...
# Ruta en local.*
curl -s -i -X POST http://localhost:8001/services/local-grafana-redirect-service/routes \
...
curl -s -i -X POST http://localhost:8001/routes/local-redirect/plugins \
  --data name=pre-function \
  --data-urlencode "config.access[1]=
    return kong.response.exit(302, nil, {
      ['Location'] = 'http://grafana-poc.test:8000'
    })"

echo "✅ Listo. Prueba en tu navegador:"
echo "➡️ http://local.grafana-poc.test:8000 → redirige a → http://grafana-poc.test:8000 (contenido servido desde :4848)"
