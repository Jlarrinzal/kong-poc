...
curl -s -i -X POST http://localhost:9001/routes \
...
curl -s -i -X POST http://localhost:9001/services/guardar-token-service/routes \
...
curl -i -X POST http://localhost:9001/routes/guardar-token/plugins \
  --data "name=jwt_validator" \
  --data "config.secret=clave-super-secreta" \
  --data "config.success_url=http://grafana-poc.test:9000" \
  --data "config.failure_url=https://example.com"

echo "✅ Listo. Visita en tu navegador:"
echo "➡️ http://local.grafana-poc.test:9000/guardar-token?token=<TU_JWT>"
